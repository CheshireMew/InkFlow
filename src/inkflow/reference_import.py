...
from inkflow.storage import Repository
...
def import_100x_library(repository: Repository, library_root: Path) -> ImportReport:
    root = library_root.resolve()
    if not (root / "Home.md").is_file():
        raise FileNotFoundError(f"100x library marker not found: {root / 'Home.md'}")

    report = ImportReport()
    _import_cases(repository, root, report)
    _import_hooks(repository, root, report)
    _import_writing_rule(repository, root, report)
    return report


def _import_cases(repository: Repository, root: Path, report: ImportReport) -> None:
    ...
    for directory, writing_format in locations:
        ...
        for path in sorted(directory.rglob("*.md")):
            try:
                ...
                _reject_existing_reference(repository, reference_id, body)
...
def _import_hooks(repository: Repository, root: Path, report: ImportReport) -> None:
...
def _import_writing_rule(repository: Repository, root: Path, report: ImportReport) -> None:
    ...
    if body.startswith("<") and body.endswith(">"):
        ...
        return
    if repository.find_rule_by_body_hash(stable_hash(body)) is not None:
        report.duplicates.append("content-writing.md: writing rule already imported")
        return
    repository.add_rule(name="100x 正式写作规则", body=body, activate=True)
...
def _reject_existing_reference(repository: Repository, reference_id: str, body: str) -> None:
    if not reference_id.strip():
        raise ValueError("missing reference id")
    existing = repository.find_reference(reference_id)
    ...
    duplicate = next(
        (
            row
            for row in repository.list_references(active_only=False)
