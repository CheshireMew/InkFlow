...
const initialForm = JSON.stringify({ name: 'OpenAI', adapter: 'openai-responses', baseUrl: 'https://api.openai.com/v1', model: '', apiKey: '', parameters: '{}' })
...
export function ProviderView({ perform }: { perform: (action: () => Promise<unknown>, message: string) => Promise<void> }) {
  ...
  const load = async () => {
    try { setItems(await api.providers()); setLoadError(null) }
    catch (error) { setLoadError(error instanceof Error ? error.message : '模型配置读取失败') }
  }
  useEffect(() => {
    let active = true
    void api.providers().then((next) => { if (active) { setItems(next); setLoadError(null) } }).catch((error: unknown) => { if (active) setLoadError(error instanceof Error ? error.message : '模型配置读取失败') })
    return () => { active = false }
  }, [])
  const dirty = open && JSON.stringify({ name, adapter, baseUrl, model, apiKey, parameters }) !== initialForm
  useUnsavedChanges('provider-form', dirty)
  const resetForm = () => {
    setName('OpenAI'); setAdapter('openai-responses'); setBaseUrl('https://api.openai.com/v1')
    setModel(''); setApiKey(''); setParameters('{}')
  }
  const toggleForm = () => {
    if (open && !confirmDiscardUnsavedChanges()) return
    resetForm()
    setOpen(!open)
  }
  ...
  return <div className="page">
    {loadError && <div className="load-error" role="alert"><span>模型配置读取失败：{loadError}</span><button className="button secondary small" onClick={() => void load()}>重试</button></div>}
    <header className="page-heading compact"><div><p className="kicker">MODEL RUNTIME</p><h1>模型与 API</h1><p>配置按版本保存。每次运行都会冻结模型、参数与能力快照，旧结果不会随设置变化。</p></div><button className="button primary" onClick={toggleForm}><Plus size={16} />{open ? '收起配置' : '新增配置'}</button></header>
