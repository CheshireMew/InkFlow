...
export function LibraryView({ perform }: { perform: (action: () => Promise<unknown>, message: string) => Promise<void> }) {
  ...
  const load = async () => setItems(await api.references(true))
  useEffect(() => { void api.references(true).then(setItems) }, [])
  ...
  const edit = async (item: ReferenceItem) => {
    ...
    setDraft({ id: full.id, kind: full.kind, title: full.title, body: full.body ?? '', formats: full.formats_json.join(', '), techniques: full.techniques_json.join(', '), active: full.active })
  }
  ...
  return <div className="page">
    <header className="page-heading compact"><div><p className="kicker">REFERENCE LIBRARY</p><h1>案例与钩子资料库</h1><p>模型选择阶段只看到编号、形式和技巧；正文只在选中后进入交接。</p></div><button className="button secondary" onClick={() => setDraft(emptyDraft)}><FilePlus2 size={16} />新建条目</button></header>
