...
export function HandoffStage({ projectId, handoff, history, run, onContinue }: { projectId: string; handoff: Handoff | null; history: Handoff[]; run: (action: () => Promise<unknown>, message: string) => Promise<void>; onContinue: () => void }) {
  ...
  const dirty = Boolean(handoff && draft && (
    draft.purified_material !== handoff.core.purified_material
    || draft.other_inputs !== handoff.core.other_inputs
    || JSON.stringify(splitBodies(cases)) !== JSON.stringify(handoff.core.reference_cases)
    || JSON.stringify(splitBodies(hooks)) !== JSON.stringify(handoff.core.reference_hooks)
  ))
  useUnsavedChanges('handoff', dirty)
